...
public class WikiBrowser extends Window implements DTarget, DropTarget, IHWindowParent {
    ...
    public WikiBrowser(Coord c, Coord sz, Widget parent) {
        ...
        search = new TextEntry(new Coord(5, 15), new Coord(sz.getX(), 20), this, "");
    ...
    public void mousemove(Coord c) {
        if (rsm) {
            ...
            doff = c;
            ssz = ssz.add(d);
            ssz.setX(Math.max(ssz.getX(), minsz.getX()));
            ssz.setY(Math.max(ssz.getY(), minsz.getY()));
            pack();
        } else {
            super.mousemove(c);
        }
    ...
    private void deltasz() {
        Coord s = ssz.sub(0, btnh + gzsz.getY() + addrh);
        for (HWindow wnd : wnds) {
            wnd.setsz(s);
        }
        search.sz = new Coord(s.getX() - 10, 20);
        updbtns();
    }
    ...
    public void addwnd(final HWindow wnd) {
        wnd.sz = ssz.sub(0, btnh + gzsz.getY() + addrh);
        wnd.c = new Coord(0, btnh + gzsz.getY() + addrh);
        wnds.add(wnd);
    ...
    private void updbtns() {
        int ws = wnds.size();
        int k = Math.max((ssz.getX() - sbtnw) / minbtnw, 1);
        if (k > (ws >> 1)) {
            k = Math.max(ws >> 1, 1);
            if ((ws % 2) != 0)
                k++;
        }
        int bw = Math.min((ssz.getX() - sbtnw) / k, maxbtnw);
        int bpp = 2 * k;
        ...
        for (int i = 0; i < bpp; i++) {
            ...
            if (woff > 0) {
                sub.visible = true;
                sub.c = btnc.add(ssz.getX() - sbtnw, 0);
            }
            if (woff < ws - bpp) {
                sdb.visible = true;
                sdb.c = btnc.add(ssz.getX() - sbtnw, 20);
            }
