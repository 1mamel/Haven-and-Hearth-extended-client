...
public class Fightview extends Widget {
    ...
    public class Relation {
        ...
        public Relation(final int gobid) {
            this.gobid = gobid;
        ...
        public Tex name() {
            final Gob gob = ui.sess.glob.oc.getgob(gobid);
            if (gob != null) {
        ...
        public void give(final int state) {
            if (this == current)
                curgive.state = state;
            this.give.state = state;
        }

        public void show(final boolean state) {
            ava.visible = state;
            give.visible = state;
        }
    ...
    public Fightview(final Coord c, final Widget parent) {
        super(c.add(-bg.sz().x, 0), new Coord(bg.sz().x, (bg.sz().y + ymarg) * height), parent);
        final SlenHud s = ui.slen;//  ui.root.findchild(SlenHud.class);
        curgive = new GiveButton(cgivec, ui.root, 0) {
            public void wdgmsg(final String name, final Object... args) {
    ...
    public void draw(final GOut g) {
        ...
        for (final Relation rel : lsrel) {
            ...
            final Tex text = RichText.render(str, 0).tex();
    ...
    public static class Notfound extends RuntimeException {
        ...
        public Notfound(final int id) {
            super("No relation for Gob ID " + id + " found");
            this.id = id;
        }
    }

    private Relation getrel(final int gobid) {
        for (final Relation rel : lsrel) {
            if (rel.gobid == gobid)
                return (rel);
        }
    ...
    private Indir<Resource> n2r(final int num) {
        if (num < 0)
            return (null);
    ...
    public void uimsg(@NotNull final String msg, final Object... args) {
        if (msg.equals("new")) {
            final Relation rel = new Relation((Integer) args[0]);
            ...
            final Relation rel = getrel((Integer) args[0]);
            rel.remove();
            lsrel.remove(rel);
            return;
