...
public class HtmlReporter {
    ...
    public static class ErrorIdentity implements Comparable<ErrorIdentity> {
        ...
        private final Pattern[] ignclass = new Pattern[]{
                Pattern.compile("sun\\.reflect\\..*"),
                Pattern.compile("java\\.lang\\.reflect\\..*"),
        };

        private static int equals(final String a, final String b) {
            if ((a == null) && (b == null))
                return (0);
            if (a == null)
                return (-1);
        ...
        public static int stcmp(final StackTraceElement a, final StackTraceElement b) {
        ...
        public int thcmp(final Throwable a, final Throwable b) {
            int sc = equals(a.getClass().getName(), b.getClass().getName());
            if (sc != 0)
                return (sc);
            final StackTraceElement[] at = a.getStackTrace();
            final StackTraceElement[] bt = b.getStackTrace();
            int ai = 0, bi = 0;
            while (true) {
                final boolean ad = ai >= at.length;
                final boolean bd = bi >= bt.length;
                if (ad && bd)
                    break;
                else if (ad && !bd)
                    return (-1);
                ...
                ign:
                while (true) {
                    for (final Pattern ignp : ignclass) {
                        final Matcher ma = ignp.matcher(at[ai].getClassName());
                        if (ma.matches()) {
                            ai++;
                            continue ign;
                        }
                        final Matcher mb = ignp.matcher(bt[bi].getClassName());
                        if (mb.matches()) {
                            bi++;
                            continue ign;
                        }
        ...
        public ErrorIdentity(final Report r) {
            if ((jarrev = (String) r.props.get("jar.git-rev")) == null)
                jarrev = "";
            t = r.t;
        }

        public int compareTo(final ErrorIdentity o) {
            final int sc = jarrev.compareTo(o.jarrev);
            if (sc != 0)
                return (sc);
            return (thcmp(t, o.t));
        }

        public boolean equals(final ErrorIdentity o) {
            return (compareTo(o) == 0);
        }
    }

    public static String htmlhead(final String title) {
    ...
    public static String htmlq(final String html) {
        if (html == null)
            return ("(null)");
        final StringBuilder buf = new StringBuilder();
    ...
    public static void makereport(final OutputStream outs, final Report rep) throws IOException {
        final PrintWriter out = new PrintWriter(new OutputStreamWriter(outs, "UTF-8"));
        ...
        final SortedSet<String> keys = new TreeSet<String>(rep.props.keySet());
        for (final String key : keys) {
            final Object val = rep.props.get(key);
            final String vals;
    ...
    public static Throwable findrootexc(final Throwable t) {
        if (t.getCause() == null)
            return (t);
        for (final Class b : boring) {
            if (t.getClass() == b)
    ...
    public static void makeindex(final OutputStream outs, final Map<File, Report> reports, final Map<File, Exception> failed) throws IOException {
        final PrintWriter out = new PrintWriter(new OutputStreamWriter(outs, "UTF-8"));
        ...
        final Map<ErrorIdentity, List<Map.Entry<File, Report>>> groups = new TreeMap<ErrorIdentity, List<Map.Entry<File, Report>>>();
        for (final Map.Entry<File, Report> rent : reports.entrySet()) {
            final ErrorIdentity id = new ErrorIdentity(rent.getValue());
            if (groups.get(id) == null)
                groups.put(id, new ArrayList<Map.Entry<File, Report>>());
            groups.get(id).add(rent);
        }
        for (final Map.Entry<ErrorIdentity, List<Map.Entry<File, Report>>> errorIdentityListEntry : groups.entrySet()) {
            out.println("<h2>" + htmlq(findrootexc(errorIdentityListEntry.getKey().t).getClass().getSimpleName()) + "</h2>");
            out.println("<table><tr>");
            out.println("    <th>File</th>");
            out.println("    <th>Time</th>");
            for (final String pn : props)
                out.println("    <th>" + htmlq(pn) + "</th>");
            out.println("</tr>");

            final List<Map.Entry<File, Report>> reps = errorIdentityListEntry.getValue();
            Collections.sort(reps, new Comparator<Map.Entry<File, Report>>() {
                public int compare(final Map.Entry<File, Report> a, final Map.Entry<File, Report> b) {
                    final long at = a.getValue().time;
                    final long bt = b.getValue().time;
                    if (at > bt)
                        return (-1);
                    else if (at < bt)
                        return (1);
            ...
            for (final Map.Entry<File, Report> rent : reps) {
                final File file = rent.getKey();
                final Report rep = rent.getValue();
                out.println("    <tr>");
                out.print("        <td>");
        ...
        if (!failed.isEmpty()) {
            ...
            for (final Map.Entry<File, Exception> fileExceptionEntry : failed.entrySet()) {
                final Exception exc = fileExceptionEntry.getValue();
                out.print("    <tr>");
                out.print("<td>" + htmlq(fileExceptionEntry.getKey().getName()) + "</td><td>" + htmlq(exc.getClass().getName()) + ": " + htmlq(exc.getMessage()) + "</td>");
                out.println("    </tr>");
            }
            out.println("</table>");
        }
    ...
    public static void main(final String[] args) throws Exception {
        final File indir = new File("/srv/haven/errors");
        final File outdir = new File("/srv/www/haven/errors");
        final Map<File, Report> reports = new HashMap<File, Report>();
        final Map<File, Exception> failed = new HashMap<File, Exception>();
        ...
        for (final Map.Entry<File, Report> fileReportEntry : reports.entrySet()) {
